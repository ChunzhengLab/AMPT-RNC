...
# 默认基础路径
DEFAULT_BASE_PATH="/home/chunzheng/AMPT-RNC/slurm_jobs/outputs"
...
check_single_file() {
    ...
    if [ "$total_events" -gt 0 ]; then
        ...
        # 计算事件处理速率
        if command -v squeue &> /dev/null && [[ "$file" =~ job_([0-9]+) ]]; then
            local job_id="${BASH_REMATCH[1]}"
            # 获取作业开始时间（SLURM格式）
            local start_time=$(squeue -j "$job_id" -h -o "%S" 2>/dev/null)
...
get_slurm_info() {
    ...
    if command -v squeue &> /dev/null; then
        # 获取运行中作业的信息 (作业名、节点、状态、运行时间)
        local slurm_info=$(squeue -j "$job_id" -h -o "%j|%N|%T|%M|%m" 2>/dev/null | head -1)
        
        if [ -n "$slurm_info" ]; then
            local job_name=$(echo "$slurm_info" | cut -d'|' -f1)
            local node=$(echo "$slurm_info" | cut -d'|' -f2)
            local state=$(echo "$slurm_info" | cut -d'|' -f3)
            local runtime=$(echo "$slurm_info" | cut -d'|' -f4)
            local memory=$(echo "$slurm_info" | cut -d'|' -f5)
            
            echo "    SLURM: $node, 状态: $state, ${memory}内存, 运行时间: $runtime"
        else
            echo "    SLURM: 作业未运行或已完成"
        fi
    fi
}
...
show_summary() {
    ...
    if command -v squeue &> /dev/null; then
        echo "SLURM作业内存使用:"
        squeue -h -o "  作业 %A: %N %m" -u $USER 2>/dev/null || echo "  无运行中作业"
        echo ""
    fi
    ...
    for file in "${files[@]}"; do
        ...
        if [ -f "$file" ]; then
            ...
            if [ "$events" -gt 0 ]; then
                ...
                # 假设100个事件为完成标准
                if [ "$events" -ge 100 ]; then
                    completed_jobs=$((completed_jobs + 1))
                fi
            fi
            
            # 尝试获取作业内存使用
            if command -v squeue &> /dev/null && [[ "$job_id" != "unknown" ]]; then
                local memory=$(squeue -j "$job_id" -h -o "%m" 2>/dev/null | sed 's/[^0-9]//g')
